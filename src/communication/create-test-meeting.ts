...
async function createTestMeeting() {
  ...
  try {
    ...
    // Create meeting URLs
    const baseUrl = 'https://app.videosdk.live/meeting';
    
    // Meeting URLs with token embedded in query params (easier for testing)
    const adminMeetingUrl = `${baseUrl}/${roomId}?token=${encodeURIComponent(adminToken)}`;
    const participantMeetingUrl = `${baseUrl}/${roomId}?token=${encodeURIComponent(participantToken)}`;
    ...
    logger.log('------------------------------------------------------------');
    logger.log('DIRECT JOIN LINKS:');
    logger.log('\nAdmin link (for joining as a moderator):');
    logger.log(adminMeetingUrl);
    logger.log('\nParticipant link (for joining as a regular user):');
    logger.log(participantMeetingUrl);
    logger.log('------------------------------------------------------------');
    logger.log('\nThese links will work for 1 hour. The room will be active for 24 hours.');
    logger.log('To join using these links, simply open them in your browser.');
    logger.log('------------------------------------------------------------\n');
