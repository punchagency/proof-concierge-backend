...
export const productionUsersSeed = async (prisma: PrismaClient): Promise<void> => {
  try {
    ...
    if ((!superAdminPassword || superAdminPassword === 'default_super_admin_password') && 
        (!adminPassword || adminPassword === 'default_admin_password')) {
      console.warn('⚠️ Using default passwords because SUPER_ADMIN_PASSWORD and/or ADMIN_PASSWORD environment variables are not set');
